...
export class AuthRepository {
  ...
  async verifyAccount(email: string): Promise<boolean> {
    const query = 'UPDATE "users" SET is_verified = TRUE, updated_at = NOW() WHERE email = $1 RETURNING *';
    const result = await this.pool.query(query, [email]);
    if (result.rows.length === 0) {
      return false;
    }
  ...
  async updatePassword(email: string, password: string): Promise<UserEntity | null> {
